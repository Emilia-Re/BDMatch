...
def split_osssl_data(args, data, targets, num_all_classes, seen_classes,
                     lb_per_class, val_per_class=0,
                     lb_index=None, ulb_index=None, val_index=None, include_lb_to_ulb=True, load_exist=True):
    """
    data & target is splitted into labeled, unlabeled and validate set.
    
    Args
        data: data to be split to labeled and unlabeled 
        targets: targets to be split to labeled and unlabeled 
        num_all_classes: number of total classes
        seen_classes: set of seen classes
        lb_per_class: number of labeled samples per class.
        val_per_class: number of labeled samples per class for validation.
        ...
        val_index: If np.array of index is given, select the data[index], target[index] as validation samples.
        include_lb_to_ulb: If True, labeled data is also included in unlabeled data
    """
    data, targets = np.array(data), np.array(targets)
    targets = reassign_target(targets, num_all_classes, seen_classes)
    lb_idx, ulb_idx, val_idx = get_data_idx(args, data, targets, len(seen_classes), 
                                            lb_per_class, val_per_class, load_exist=load_exist)
    ...
    return data[lb_idx], targets[lb_idx], data[ulb_idx], targets[ulb_idx], data[val_idx], targets[val_idx]


def reassign_target(targets, num_all_classes, seen_classes):
    targets = np.array(targets)

    all_classes = set(range(num_all_classes))
    unseen_classes = all_classes - seen_classes
    targets_new = np.ones_like(targets) * (-1)  # assign new labels for OSSL, first k classes are seen-class

    for i, lbi in enumerate(seen_classes):
        all_lbi_indices = np.where(targets == lbi)[0]
...
def get_data_idx(args, data, target, num_classes,
                 lb_per_class, val_per_class=0, load_exist=True):
    '''
    (sampling with balanced ratio over classes)
    '''
    dump_dir = os.path.join(base_dir, 'data', args.dataset, 'labeled_idx')
    os.makedirs(dump_dir, exist_ok=True)
    lb_dump_path = os.path.join(dump_dir, f'lb_num_classes{args.num_classes}_labels{args.lb_per_class}_val{args.val_per_class}_seed{args.seed}_idx.npy')
    ulb_dump_path = os.path.join(dump_dir, f'ulb_num_classes{args.num_classes}_labels{args.lb_per_class}_val{args.val_per_class}_seed{args.seed}_idx.npy')
    val_dump_path = os.path.join(dump_dir, f'val_num_classes{args.num_classes}_labels{args.lb_per_class}_val{args.val_per_class}_seed{args.seed}_idx.npy')

    if os.path.exists(lb_dump_path) and os.path.exists(ulb_dump_path) and os.path.exists(val_dump_path) and load_exist:
        lb_idx = np.load(lb_dump_path)
        ulb_idx = np.load(ulb_dump_path)
        val_idx = np.load(val_dump_path)
    ...
    for c in range(num_classes):
        ...
        lb_idx.extend(idx[:lb_per_class])
        val_idx.extend(idx[lb_per_class:lb_per_class+val_per_class])
    ulb_idx = set(range(len(data))) - set(lb_idx) - set(val_idx)
    ulb_idx = np.array(sorted(list(ulb_idx)))
