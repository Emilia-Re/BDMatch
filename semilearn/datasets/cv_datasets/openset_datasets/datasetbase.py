        ...
        if self.transform is None:
            ...
            if not self.is_ulb:
                ...
                if self.alg == 'fullysupervised' or self.alg == 'supervised':
                    return_dict.update({'idx_ulb': idx})
                    return return_dict
                elif self.alg == 'pseudolabel' or self.alg == 'vat':
                    return_dict.update({'idx_ulb': idx, 'x_ulb_w':img_w})
                    return return_dict
                elif self.alg == 'pimodel' or self.alg == 'meanteacher' or self.alg == 'mixmatch':
                    # NOTE x_ulb_s here is weak augmentation
                    return_dict.update({'idx_ulb': idx, 'x_ulb_w': img_w, 'x_ulb_s': self.transform(img)})
                    return return_dict
                    ...
                    return_dict.update({'idx_ulb': idx, 'x_ulb_w': img_w, 'x_ulb_s_0': img_s1, 'x_ulb_s_1':img_s2, 'x_ulb_s_0_rot':img_s1_rot, 'rot_v':rotate_v_list.index(rotate_v1)})
                    return return_dict
                elif self.alg == 'comatch':
                    return_dict.update({'idx_ulb': idx, 'x_ulb_w': img_w, 'x_ulb_s_0': self.strong_transform(img), 'x_ulb_s_1':self.strong_transform(img)})
                    return return_dict
                else:
                    return_dict.update({'idx_ulb': idx, 'x_ulb_w': img_w, 'x_ulb_s': self.strong_transform(img)})
                    return return_dict
