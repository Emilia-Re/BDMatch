...
def get_imagenet30(args, alg, name, labeled_percent, num_classes, data_dir='./data'):
    ...
    if alg == 'fullysupervised':
        lb_dset = IN30Dataset(root=os.path.join(data_dir, "one_class_train"),
                              ...
                              is_ulb=False,
                              alg=alg,
                              flist=os.path.join(data_dir, 'filelist/train_unlabeled_full.txt'))
    else:
        lb_dset = IN30Dataset(root=os.path.join(data_dir, "one_class_train"),
                              transform=transform_weak,
                              is_ulb=False,
    ...
    if args.ulb_purified: # unlabeled data only has seen classes
        seen_indices = np.where(ulb_dset.targets < num_classes)[0]
        ulb_dset.data, ulb_dset.targets = ulb_dset.data[seen_indices], ulb_dset.targets[seen_indices]
    ...
    test_dset = IN30Dataset(root=os.path.join(data_dir, "one_class_test"),
                            ...
                            is_ulb=False,
                            alg=alg,
                            flist=os.path.join(data_dir, 'filelist/test.txt'))

    test_targets = test_dset.targets
