...
from semilearn.os_algorithms.hooks import OSEvaluationHook
...
class AlgorithmBase:
    ...
    def __init__(
        ...
        **kwargs):
        ...
        self.algorithm = args.algorithm if args.os_algorithm is None else args.os_algorithm
    ...
    def set_ema_model(self):
        ...
        """
        ema_model = EMA(self.model, self.ema_m)
        return ema_model
    ...
    def evaluate(self, eval_dest='eval', out_key='logits', return_logits=False):
        ...
        with torch.no_grad():
            for data in eval_loader:
                ...
                logits = self.ema_model.model(x)[out_key]
    ...
    def get_save_dict(self):
        ...
        save_dict = {
            ...
            'ema_model': self.ema_model.model.state_dict(),
    ...
    def load_model(self, load_path):
        ...
        self.ema_model.model.load_state_dict(checkpoint['ema_model'])
...
class OSAlgorithmBase(AlgorithmBase):
    def __init__(self, args, net_builder, tb_log=None, logger=None, **kwargs):
        super().__init__(args, net_builder, tb_log, logger, **kwargs)
        self.OOD_threshold = args.OOD_threshold
        
        self.algorithm = args.os_algorithm
    ...
    def set_data_loader(self):
        ...
        if self.dataset_dict['test'] is not None:
            loader_dict['test'] = {}
            loader_dict['test']['id_dset'] = get_data_loader(self.args,
                                                             self.dataset_dict['test']['id_dset'],
                                                             self.args.eval_batch_size,
                                                             # make sure data_sampler is None for evaluation
                                                             data_sampler=None,
                                                             num_workers=self.args.num_workers,
                                                             drop_last=False)
            if self.dataset_dict['test']['ood_dset'] is not None:
                loader_dict['test']['ood_dset'] = {}
                for k, v in self.dataset_dict['test']['ood_dset'].items():
                    loader_dict['test']['ood_dset'][k] = get_data_loader(self.args,
                                                                         self.dataset_dict['test']['ood_dset'][k],
                                                                         self.args.eval_batch_size,
                                                                         # make sure data_sampler is None for evaluation
                                                                         data_sampler=None,
                                                                         num_workers=self.args.num_workers,
    ...
    def set_hooks(self):
        ...
        self.register_hook(ParamUpdateHook(), None, "HIGHEST")
        self.register_hook(EMAHook(), None, "HIGH")
        self.register_hook(OSEvaluationHook(), None, "HIGH")
        self.register_hook(CheckpointHook(), None, "HIGH")
        self.register_hook(DistSamplerSeedHook(), None, "NORMAL")
        self.register_hook(TimerHook(), None, "LOW")
        self.register_hook(LoggingHook(), None, "LOWEST")
    ...
    def evaluate(self, eval_dest='eval', out_key='logits', return_logits=False):
        ...
        if eval_dest == 'eval':
            ...
            cls_target_list = []
            cls_pred_list = []
            if self.loader_dict['test']['ood_dset'] is not None:
                for k, v in self.loader_dict['test']['ood_dset'].items():
                    AUC, targets, scores, balanced_top1, y_true, y_pred = self.evaluate_ood(v, seen_OOD_targets, seen_ID_scores, seen_cls_targets, seen_cls_pred, out_key)
                    
                    OOD_targets = np.concatenate([OOD_targets, targets])
                    ID_scores = np.concatenate([ID_scores, scores])
                    target_list.append(targets)
                    score_list.append(scores)
                    eval_dict['test/AUC_{}'.format(k)] = AUC
                    
                    cls_targets = np.concatenate([cls_targets, y_true])
                    cls_pred = np.concatenate([cls_pred, y_pred])
                    cls_target_list.append(y_true)
                    cls_pred_list.append(y_pred)
                    eval_dict['test/top-1-balanced_acc_overall_{}'.format(k)] = balanced_top1
                
                over_AUC = roc_auc_score(OOD_targets, 1 - ID_scores)
                eval_dict['test/AUC_overall'] = over_AUC
                
                over_balanced_top1 = balanced_accuracy_score(cls_targets, cls_pred)
                eval_dict['test/top-1-balanced_acc_overall'] = over_balanced_top1
                
                target_list.append(seen_OOD_targets)
                score_list.append(seen_ID_scores)
                target_list = np.concatenate(target_list)
                score_list = np.concatenate(score_list)
                over_OOD_AUC = roc_auc_score(target_list, 1 - score_list)
                eval_dict['test/AUC_OOD_overall'] = over_OOD_AUC
                
                cls_target_list.append(seen_cls_targets)
                cls_pred_list.append(seen_cls_pred)
                cls_target_list = np.concatenate(cls_target_list)
                cls_pred_list = np.concatenate(cls_pred_list)
                over_OOD_balanced_top1 = balanced_accuracy_score(cls_target_list, cls_pred_list)
                eval_dict['test/top-1-balanced_acc_OOD_overall'] = over_OOD_balanced_top1
            return eval_dict

    def evaluate_eval(self, out_key='logits', return_logits=False):
        self.model.eval()
        # self.ema.apply_shadow()
        ...
        y_true, y_pred, y_logits = self.evaluate_on_dset(eval_loader, out_key, False)
        
        loss = self.compute_sup_loss(y_logits, y_true)
        y_true = y_true.numpy()
        y_pred = y_pred.numpy()
        ...
        eval_dict = {eval_dest+'/loss': loss, eval_dest+'/top-1-acc': top1, 
                     eval_dest+'/balanced_acc': balanced_top1, eval_dest+'/precision': precision, eval_dest+'/recall': recall, eval_dest+'/F1': F1}
        if return_logits:
            eval_dict[eval_dest+'/logits'] = y_logits
        return eval_dict
    
    def evaluate_test(self, out_key='logits', return_logits=False):
        self.model.eval()
        # self.ema.apply_shadow()
        
        eval_dest='test'
        eval_loader = self.loader_dict[eval_dest]['id_dset']
        
        y_true, y_pred, y_logits, ID_scores = self.evaluate_on_dset(eval_loader, out_key, True)
        
        seen_idx = y_true < self.num_classes
        seen_y_true = y_true[seen_idx]
        seen_y_logits = y_logits[seen_idx]
        ID_loss = self.compute_sup_loss(seen_y_logits, seen_y_true)
        
        y_true = y_true.numpy()
        y_pred = y_pred.numpy()
        y_logits = y_logits.numpy()
        ID_scores = ID_scores.numpy()
        seen_idx = seen_idx.numpy()
        seen_y_pred = y_pred[seen_idx]
        seen_y_true = seen_y_true.numpy()
        seen_y_logits = seen_y_logits.numpy()
        ID_top1 = accuracy_score(seen_y_true, seen_y_pred)
        ID_balanced_top1 = balanced_accuracy_score(seen_y_true, seen_y_pred)
        ID_precision = precision_score(seen_y_true, seen_y_pred, average='macro')
        ID_recall = recall_score(seen_y_true, seen_y_pred, average='macro')
        ID_F1 = f1_score(seen_y_true, seen_y_pred, average='macro')
        
        cf_mat = confusion_matrix(seen_y_true, seen_y_pred, normalize='true')
        self.print_fn('confusion matrix:\n' + np.array_str(cf_mat))
        
        y_true[~seen_idx] = self.num_classes
        OOD_pred = ID_scores < self.OOD_threshold
        y_pred[OOD_pred] = self.num_classes
        over_top1 = accuracy_score(y_true, y_pred)
        over_balanced_top1 = balanced_accuracy_score(y_true, y_pred)
        
        OOD_targets = ~seen_idx
        
        AUC = roc_auc_score(OOD_targets, 1 - ID_scores)
        
        # self.ema.restore()
        self.model.train()

        eval_dict = {eval_dest+'/loss': ID_loss, eval_dest+'/top-1-acc_ID': ID_top1, 
                     eval_dest+'/balanced_acc_ID': ID_balanced_top1, eval_dest+'/precision_ID': ID_precision,
                     eval_dest+'/recall_ID': ID_recall, eval_dest+'/F1_ID': ID_F1,
                     eval_dest+'/top-1-acc_overall_{}'.format(self.args.dataset): over_top1,
                     eval_dest+'/top-1-balanced_acc_overall_{}'.format(self.args.dataset): over_balanced_top1,
                     eval_dest+'/AUC_{}'.format(self.args.dataset): AUC}
        if return_logits:
            eval_dict[eval_dest+'/logits'] = y_logits
        return eval_dict, OOD_targets, ID_scores, OOD_targets[seen_idx], ID_scores[seen_idx], y_true, y_pred, y_true[seen_idx], y_pred[seen_idx]
    
    def evaluate_ood(self, eval_loader, seen_OOD_targets, seen_ID_scores, seen_cls_targets, seen_cls_pred, out_key='logits'):
        self.model.eval()
        ...
        y_pred = y_pred.numpy()
        y_logits = y_logits.numpy()
        OOD_targets = y_true >= 0
        y_true[:] = self.num_classes
        OOD_pred = ID_scores < self.OOD_threshold
        y_pred[OOD_pred] = self.num_classes
        
        eval_OOD_targets = np.concatenate([seen_OOD_targets, OOD_targets])
        eval_ID_scores = np.concatenate([seen_ID_scores, ID_scores])
        AUC = roc_auc_score(eval_OOD_targets, 1 - eval_ID_scores)
        
        eval_cls_targets = np.concatenate([seen_cls_targets, y_true])
        eval_cls_pred = np.concatenate([seen_cls_pred, y_pred])
        over_balanced_top1 = balanced_accuracy_score(eval_cls_targets, eval_cls_pred)
        return AUC, OOD_targets, ID_scores, over_balanced_top1, y_true, y_pred
        
    def evaluate_on_dset(self, eval_loader, out_key, com_ID_score=False):
        y_true = []
        y_pred = []
        # y_probs = []
        y_logits = []
        with torch.no_grad():
            for data in eval_loader:
                ...
                y_true.append(y.cpu())
                y_pred.append(torch.max(logits, dim=-1)[1].cpu())
        ...
        if com_ID_score:
            ID_scores = self.compute_ID_score(y_logits)
            return y_true, y_pred, y_logits, ID_scores
        else:
            return y_true, y_pred, y_logits
    
    def compute_ID_score(self, logits, logits_open=None):
        y_probs = self.compute_prob(logits)
        ID_scores = y_probs.max(dim=1)[0]
        return ID_scores
    
    def compute_sup_loss(self, logits, targets):
        return F.cross_entropy(logits, targets.long(), reduction='mean', ignore_index=-1)