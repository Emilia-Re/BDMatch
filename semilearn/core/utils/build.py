...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    """
    from semilearn.datasets import get_openset_cifar, get_ood_dset, get_imagenet30
    
    # openset dataset
    if dataset in ["cifar10_openset", "cifar100_openset"]:
        lb_dset, ulb_dset, eval_dset, self_test_dset = get_openset_cifar(args, algorithm, dataset, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        if args.ood_test:
            ood_dset = get_ood_dset(args, algorithm, dataset, num_classes, data_dir=data_dir)
        else:
            ood_dset = None
        test_dset = {}
        test_dset['id_dset'] = self_test_dset
        test_dset['ood_dset'] = ood_dset
    elif dataset in ["imagenet30"]:
        lb_dset, ulb_dset, eval_dset, self_test_dset = get_imagenet30(args, algorithm, dataset, args.labeled_percent, num_classes, data_dir=data_dir)
        if args.ood_test:
            ood_dset = get_ood_dset(args, algorithm, dataset, num_classes, data_dir=data_dir)
        else:
            ood_dset = None
